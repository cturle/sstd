...
CONSTRAINT PK_ADRC PRIMARY KEY (ADRC_ID)
)
...
CONSTRAINT PK_ASSI PRIMARY KEY (ASSI_ID)
)
...
CONSTRAINT PK_ASSU PRIMARY KEY (ASSU_ID)
)
...
RIS_CRIS int,
ADRC_CRIS int,
CONSTRAINT PK_CRIS PRIMARY KEY (CRIS_ID)
)
...
CON_DIS int,
CONSTRAINT PK_DIS PRIMARY KEY (DIS_ID)
)
...
GAR_MCO int,
RIS_GAR int,
CONSTRAINT PK_GAR PRIMARY KEY (GAR_ID)
)
...
INT_ADR int,
PER_INT int,
CONSTRAINT PK_INT PRIMARY KEY (INT_ID)
)
...
CONSTRAINT PK_MCO PRIMARY KEY (MCO_ID)
)
...
RIS_MCO int,
CON_RIS int,
CONSTRAINT PK_RIS PRIMARY KEY (RIS_ID)
)
...
BEN_ADRT int,
BEN_PER int,
CONSTRAINT PK_BEN PRIMARY KEY (BEN_ID)
)
...
DOS_ENT int,
DOS_REF int,
CONSTRAINT PK_DOS PRIMARY KEY (DOS_ID)
)
...
OFF_VEN int,
DOS_OFF int,
CONSTRAINT PK_OFF PRIMARY KEY (OFF_ID)
)
...
PER_PAMC int,
CONSTRAINT PK_PMAC PRIMARY KEY (PMAC_ID)
)
...
PASS_VOFF int,
CONSTRAINT PK_VOF PRIMARY KEY (VOF_ID)
)
...
CONSTRAINT PK_ADRP PRIMARY KEY (ADRP_ID)
)
...
CONSTRAINT PK_ADRR PRIMARY KEY (ADRR_ID)
)
...
CONSTRAINT PK_AGN PRIMARY KEY (AGN_ID)
)
...
COV_TER int,
CONSTRAINT PK_COV PRIMARY KEY (COV_ID)
)
...
COV_IPRO int,
CONSTRAINT PK_IPRO PRIMARY KEY (IPRO_ID)
)
...
typeRemuneration int,
CONSTRAINT PK_PROT PRIMARY KEY (PROT_ID)
)
...
TER_NOM varchar(255),
CONSTRAINT PK_TER PRIMARY KEY (TER_ID)
)
...
VEN_ADRT int,
DEM_VEN int,
CONSTRAINT PK_VEN PRIMARY KEY (VEN_ID)
)
...
CONSTRAINT PK_ASGR PRIMARY KEY (ASGR_ID)
)
...
CONSTRAINT PK_ASS PRIMARY KEY (ASS_ID)
)
...
COU_CON int,
CONSTRAINT PK_CONT PRIMARY KEY (CONT_ID)
)
...
PRO_COU int,
CONSTRAINT PK_COU PRIMARY KEY (COU_ID)
)
...
GRIS_ASGR int,
RIS_GAR int,
CONSTRAINT PK_GRIS PRIMARY KEY (GRIS_ID)
)
...
GRO_LIB varchar(255),
SIN_GRO int,
CONSTRAINT PK_GRO PRIMARY KEY (GRO_ID)
)
...
GRIS_NFG int,
CONSTRAINT PK_NFG PRIMARY KEY (NFG_ID)
)
...
RISO_ADR int,
PRO_RISO int,
CONSTRAINT PK_RISO PRIMARY KEY (RISO_ID)
)
...
SIG_GRO int,
PER_SIG int,
CONSTRAINT PK_SIG PRIMARY KEY (SIG_ID)
)
...
CONSTRAINT PK_SIN PRIMARY KEY (SIN_ID)
)
...
CON_TCN int,
CONSTRAINT PK_TCN PRIMARY KEY (TCN_ID)
)
...
COU_TRN int,
CONSTRAINT PK_TCON PRIMARY KEY (TCON_ID)
)
...
RIS_TRN int,
CONSTRAINT PK_TRN PRIMARY KEY (TRN_ID)
)
...
CONSTRAINT PK_VASS PRIMARY KEY (VASS_ID)
)
...
VRIS_VCR int,
CONSTRAINT PK_VCR PRIMARY KEY (VCR_ID)
)
...
VER_CON int,
CONSTRAINT PK_VCO PRIMARY KEY (VCO_ID)
)
...
VER_VDIS int,
CONSTRAINT PK_VDIS PRIMARY KEY (VDIS_ID)
)
...
VGA_VASS int,
VRIS_VGA int,
CONSTRAINT PK_VGA PRIMARY KEY (VGA_ID)
)
...
VRIS_ADR int,
VER_VRIS int,
CONSTRAINT PK_VRIS PRIMARY KEY (VRIS_ID)
)
...
add constraint FK_CONTRIBUTION_CONT_COUVERTURE_COU
foreign key (COU_CON)
...
add constraint FK_TARIFCONTRIBUTIONPARNIVEAUFRANCHISE_TCN_CONTRIBUTION_CONT
foreign key (CON_TCN)
references instruction.CONTRIBUTION_CONT

alter table instruction.TARIFCOUVERTUREPARNIVEAUFRANCHISE_TCON
add constraint FK_TARIFCOUVERTUREPARNIVEAUFRANCHISE_TCON_COUVERTURE_COU
foreign key (COU_TRN)
references instruction.COUVERTURE_COU

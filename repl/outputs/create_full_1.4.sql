...
create table affaire.ADRESSE_ADR (
PK_ADR int identity not null,
ADR_ADRESSE varchar(255),
ADR_CODEINSEE varchar(255),
ADR_CP varchar(255),
ADR_COMPLEMENTADR varchar(255),
ADR_LIBCOMMUNE varchar(255),
ADR_LIBVOIE varchar(255),
ADR_NUMVOIE varchar(255),
ADR_PAYS varchar(255),
...
create table commun.ADRESSE_ADR (
PK_ADR int identity not null,
ADR_ADRESSE varchar(255),
...
create table commun.ADRESSE_ADRT (
...
add constraint FK_CARACTERISTIQUERISQUEACCEPTATION_CRIS_RISQUEACCEPTATION_RIS
...
references affaire.ADRESSE_ADR

alter table acceptation.INTERLOCUTEUR_INT
add constraint FK_INTERLOCUTEUR_INT_PERSONNEACCEPTATION_PER
...
add constraint FK_RISQUEACCEPTATION_RIS_ADRESSE_ADR
foreign key (FK_RIS_ADR)
references commun.ADRESSE_ADR
...
alter table acceptation.RISQUEACCEPTATION_RIS
add constraint FK_RISQUEACCEPTATION_RIS_CONTRAT_CON
...
references commun.ADRESSE_ADRT
...
alter table affaire.INSTRUCTION_INS
add constraint FK_INSTRUCTION_INS_DOSSIER_DOS
...
alter table affaire.INTERLOCUTEUR_INT
add constraint FK_INTERLOCUTEUR_INT_DEMANDEUR_DEM
foreign key (FK_DEM_INT)
references affaire.DEMANDEUR_DEM

alter table affaire.INTERLOCUTEUR_INT
add constraint FK_INTERLOCUTEUR_INT_BENEFICIAIRE_BEN
...
add constraint FK_DEPARTEMENT_DEP_TERRITOIRE_INTERVENTION_TER
foreign key (FK_DEP_TER)
references commun.TERRITOIRE_INTERVENTION_TER

alter table commun.INFORMATIONPRODUIT_IPRO
add constraint FK_INFORMATIONPRODUIT_IPRO_CONVENTION_COV
...
add constraint FK_DISPOSITIONPARTICULIERE_DIS_PROPOSITIONSTANDARD_PRO
...
add constraint FK_PERIODEOBSERVATIONSINISTRALITE_PER_SINISTRALITE_SIN
...
alter table instruction.RISQUEOBJET_RISO
add constraint FK_RISQUEOBJET_RISO_PROPOSITIONSTANDARD_PRO
...
alter table instruction.RISQUE_RIS
add constraint FK_RISQUE_RIS_COUVERTURE_COU
...
alter table instruction.SINISTRALITEPARGROUPEGARANTIE_SIG
add constraint FK_SINISTRALITEPARGROUPEGARANTIE_SIG_PERIODEOBSERVATIONSINISTRALITE_PER
...
add constraint FK_TARIFCONTRIBUTIONPARNIVEAUFRANCHISE_TCN_CONTRIBUTION_CON
...
add constraint FK_VERSIONDISPOSITIONPARTICULIERE_VDIS_VERSIONINSTRUCTION_VER
...
alter table instruction.VERSIONGARANTIE_VGA
add constraint FK_VERSIONGARANTIE_VGA_VERSIONRISQUE_VRIS
...
alter table instruction.VERSIONINSTRUCTION_VER
add constraint FK_VERSIONINSTRUCTION_VER_PROPOSITIONSTANDARD_PRO
...
alter table instruction.VERSIONRISQUE_VRIS
add constraint FK_VERSIONRISQUE_VRIS_VERSIONINSTRUCTION_VER
